...
test("POSIX wrapper 应使用真实 Node.js（无 ELECTRON_RUN_AS_NODE）并注入 OPENCLAW_NO_RESPAWN", () => {
  const script = buildPosixWrapperForPaths("/Applications/RunJianClaw/node", "/Applications/RunJianClaw/openclaw.mjs");
...
test("Windows wrapper 应使用真实 Node.js（无 ELECTRON_RUN_AS_NODE）并注入 OPENCLAW_NO_RESPAWN", () => {
  const script = buildWinWrapperForPaths("C:\\RunJianClaw\\node.exe", "C:\\RunJianClaw\\openclaw.mjs");
...
test("Windows PATH 脚本中的 try/catch 不能被分号打断", () => {
  const script = buildWinPathEnvScript("add", "C:\\Users\\admin\\AppData\\Local\\RunJianClaw\\bin");
...
test("Windows CLI 目录解析应同时返回当前路径与旧版迁移路径", () => {
  ...
  assert.equal(dirs.currentBinDir, "C:\\Users\\admin\\AppData\\Local\\RunJianClaw\\bin");
