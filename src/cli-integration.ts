...
// Wrapper 脚本中的标记字符串，用于识别由 RunJianClaw 生成的文件。
const CLI_MARKER = "RunJianClaw CLI";

// rc 注入块标记，安装可幂等覆盖，卸载可精确移除。
const RC_BLOCK_START = "# >>> RunJianClaw-cli >>>";
const RC_BLOCK_END = "# <<< RunJianClaw-cli <<<";
...
export function resolveWinCliBinDirsForPaths(localAppDataDir: string, userStateDir: string): WinCliBinDirs {
  return {
    currentBinDir: path.win32.join(localAppDataDir, "RunJianClaw", "bin"),
...
// 从旧版 sidecar 文件迁移 CLI 偏好到 RunJianClaw.config.json
function migrateLegacyCliPreference(): boolean | undefined {
  ...
  try {
    ...
    if (raw && typeof raw === "object" && raw.version === 1 && typeof raw.enabled === "boolean") {
      ...
      // 写入 RunJianClaw.config.json 并删除旧文件
...
// 持久化 CLI 偏好到 RunJianClaw.config.json
...
export function buildPosixWrapperForPaths(nodeBin: string, entry: string): string {
  ...
  const lines = [
    ...
    `APP_ENTRY="${safeEntry}"`,
    'if [ ! -f "$APP_NODE" ]; then',
    '  echo "Error: RunJianClaw not found at $APP_NODE" >&2',
    "  exit 127",
    "fi",
  ];
  ...
  if (!asar) {
    lines.push(
      'if [ ! -f "$APP_ENTRY" ]; then',
      '  echo "Error: RunJianClaw entry not found at $APP_ENTRY" >&2',
      "  exit 127",
      "fi",
    );
  }
...
export function buildWinWrapperForPaths(nodeBin: string, entry: string, opts?: { electronMode?: boolean }): string {
  ...
  lines.push(
    ...
    "  echo Error: RunJianClaw Node runtime not found. 1>&2",
    "  exit /b 127",
    ")",
  );
  ...
  if (!asar) {
    lines.push(
      ...
      "  echo Error: RunJianClaw entry not found. 1>&2",
      "  exit /b 127",
      ")",
    );
  }
...
// 判断指定 wrapper 是否由 RunJianClaw 管理，避免误删用户自定义脚本。
...
// 构建 RunJianClaw 管理的 rc 注入块，使用绝对路径避免与状态目录配置脱节。
...
// 从 rc 文本移除 RunJianClaw 管理块，仅删除带完整标记的块，避免误伤用户自定义行。
...
// 向 rc 文件幂等写入 RunJianClaw 管理块，重复安装不会产生重复内容。
...
// 从 rc 文件移除 RunJianClaw 管理块，仅处理本程序写入的标记块。
